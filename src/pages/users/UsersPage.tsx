        ...
        {isAdmin && pendingUsers.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-gold-100 text-forest-700 text-xs font-medium">{pendingUsers.length}</span>
            </div>
                ...
                <Card key={u.id} hover>
                  <CardBody>
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gold-50 border-2 border-gold-200 flex items-center justify-center text-forest-600 font-bold text-lg shrink-0">
                        <Clock size={20} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-sm font-semibold text-stone-800 truncate">{u.name}</h3>
          ...
          {users.map((u) => {
            ...
            return (
              <Card key={u.id} hover>
                <CardBody>
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-gold-100 flex items-center justify-center text-forest-700 font-bold text-lg shrink-0">{u.name.charAt(0)}</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-stone-800 truncate">{u.name}</h3>
                  ...
                  {horses.length > 0 && (
                    <div className="mt-3 pt-2 border-t border-cream-200">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {horses.map((h) => (
                          <span key={h.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gold-50 text-forest-700 text-xs font-medium">
                            {h.name} <span className="text-gold-400 font-mono text-[10px]">{h.passportId}</span>
                          </span>
                        ))}
                      </div>
                    </div>
